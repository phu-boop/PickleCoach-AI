...
const DetailCoach = () => {
  ...
  useEffect(() => {
    ...
    const fetchSchedule = async () => {
      try {
        const res = await getScheduledSessions(id);
        setScheduleList(res);
      } catch (err) {
      ...
      <>
        <div className="mt-30 max-w-6xl mx-auto py-10 px-4 flex flex-col md:flex-row gap-10">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="mt-6">
                <h2 className="text-xl font-bold text-[#0a0b3d] mb-2 font-grandstander">Level</h2>
                <div className="flex flex-wrap gap-2">
                <span className="bg-[#2d93ad] text-white px-3 py-1 rounded-full text-sm font-grandstander">
                  {coach.level || 'Not specified'}
                </span>
                </div>
              </div>
            </div>
        ...
        <div className="mt-20 container-main border-t-4 border-dotted border-[#A2DFFF]">
          ...
          <div className="mt-16 max-w-6xl mx-auto px-4 mb-20">
            ...
            <WeeklySchedule scheduleList={scheduleList} id_coach={id} coachLevel={level} />
          </div>
        </div>
        {/* Action Buttons */}
        {sessionStorage.getItem('role') === 'ROLE_coach' && (
            <div className="flex flex-wrap gap-4 ml-40 mb-30 mt-20">
              <button
                  onClick={() => navigate(`/Detail_coach/${sessionStorage.getItem('id_user')}`)}
